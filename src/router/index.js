...
//修正Vue原型上的push和replace方法,解决重复跳转时报错promise未捕捉的问题
// 缓存原本的push方法
...
router.beforeEach(async (to, from, next) => {
  ...
  if(token){
    ...
    if(to.path === '/login'){
      ...
      let hasUserInfo = !!store.state.user.userInfo.nickName     //  !! 转成布尔值
      if(hasUserInfo){
        ////此时代表登录了，去的不是登录页，且用户信息已经存在,直接无条件放行即可
        next()
      }else{
        //此时代表登录了，去的不是登录页，但是用户信息不存在  那我们要根据token发请求获取用户的真证信息
        try {
          await store.dispatch('getUserInfo')
          next()
        } catch (error) {
          alert('用户的token过期')
          //用户的token有问题,直接重置身份信息
    ...
    // 1、交易相关的/trade    支付相关的/pay   用户中心相关的/center  都要登录才能访问
    //.indexOf:判断路径中是否包含/trade,返回值为0表明路径中/trade是开头      .startsWith判断是否以/pay开头
