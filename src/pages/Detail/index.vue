          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 
                  input事件  输入的时候同步触发
                  blur事件   失去焦点的时候就触发,不管内容是否更新
                  change事件 失去焦点的时候就出发,但是内容必须变化
                 -->
...
<script>
  ...
  export default {
    ...
    //一般在beforeMount中处理数据,处理完成后后期就可以直接使用
    beforeMount() {
    ...
    methods: {
      ...
      //添加购物车按钮点击之后的逻辑
      async addShopCart(){
        //发请求,让后台给数据库的购物车数据表当中添加一条数据
        //根据添加成功还是失败来决定要不要跳转到添加购物车成功页面
        const {skuId,skuNum}=this
        //dispatch 分发,就是调用响应的actions函数
        //this.$store.dispatch('addOrUpdateCart',{skuId,skuNum}) 可以调用action,其结果就是actions当中异步函数的返回值,一定是一个promise
        ...
        try {
          ...
          //添加成功,跳转   编程式导航跳转
          //下个添加购物车成功页面,需要传递商品数量和商品详情信息
          //商品数量的数据简单,但是商品详情数据复杂
          //如果碰到的数据是简单数据,考虑路由褚穿参
          //如果碰到的数据时复杂数据,考虑存储手段
          //localStorage setItem getItem removeItem clear         localStorage是永久存储
          //sessionStorage   setItem getItem removeItem clear     sessionStorage浏览器关闭就没了      这里使用sessionStorage更合适
