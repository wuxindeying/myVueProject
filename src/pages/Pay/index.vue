      ...
      try {
        ...
        this.$alert(`<img src="${imgUrl}" />`, "请使用微信扫码支付", {
          ...
          beforeClose:(action, instance, done) => {
            ...
            //instance 代表当前弹出的消息盒子实例   几乎不用
            //done  就是给我们的开关   是一个函数   用于关闭 MessageBox 实例
            if(action === 'confirm'){
              //点击的确定
              // if(!this.payStatus){
              ...
              //点击的取消
              this.$message.warning('请联系客服进行反馈')
              clearInterval(this.timer);
              this.timer = null;
              done()
            }
            ...
            if (!this.timer) {
              this.timer=setInterval(async () => {
                ...
                if (result.code===200) {
                  ...
                  this.payStatus=200
                  clearInterval(this.timer)
                  this.timer=null   //clearInterval清除定时器只是吧定时器模块清楚了,this.timer中并不为空
