...
//import {reqSubmitOrder} from '@/api'    这里试着不使用vuex,改用其他方式
  export default {
    name: 'Trade',
    data() {
      return {
    ...
    methods: {
      ...
      async submitOrder(){
        ...
        try {
        const result = await this.$API.reqSubmitOrder(tradeNo,tradeData)     // 这里试着不使用vuex,改用将请求挂在Vue原型上的方式
        if(result.code === 200){
