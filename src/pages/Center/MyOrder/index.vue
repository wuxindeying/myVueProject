    ...
    <div class="order-content">
        ...
        >
          ...
          <tbody>
            ...
            >
              ...
              <!-- template是h5新加的一个无意义的标签   (这里不能用div标签来包,会出错)
              这个标签不是html标签，它是没有任何语义的，它不会影响原有的html结构
                通过 v-if 来设置合并的单元格只有一次-->
      ...
      <div class="choose-order">
        ...
        <!--  @size-change="handleSizeChange"
          ...
          -->
          <!-- layout是用来调整布局顺序的 分页器内各个东西布局按照layout的顺序,其中 '->' :用来设置位置 -->
        <el-pagination
          background
          :current-page="page"
          :page-size="limit"
        ...
        <!-- <MyPagination    //Pagination 需要四个数据一个事件
        :currentPage="page"
        :total="total"
        :pageSize="limit"
        :showPageNo="5"
        @currentChange="getMyOrderInfo"   //点击切换页码的函数
        /> -->
  ...
  methods: {
    async getMyOrderInfo(page = 1) {
      ...
      if (result.code === 200) {
        ...
        this.myOrderList = result.data.records;//拿到具体的数据
        this.total = result.data.total;//拿到总数
      }
    },
