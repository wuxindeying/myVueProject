  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <!--selector setTrademark传入子组件props,自定义addProp事件实现子向父通信-->
        ...
        <div class="details clearfix">
              ...
              <ul class="sui-nav">
                ...
                <li :class="{active: orderArr[0]==='2'}" @click="setOrder('2')">
                  <a href="javascript:">
                    ...
                    <i class="iconfont" 
                      :class="orderArr[1]==='desc' ? 'icondown' : 'iconup'"
                      v-if="orderArr[0]==='2'"></i><!-- 使用iconfont引入的图标 -->
                  </a>
                </li>
              </ul>
            </div>
          ...
          /><!-- 自定义事件currentChange, 事件的方法为getShopList-->
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        options: {
          ...
          order: '2:asc', // 排序方式 1: 综合,2: 价格,3:销量 asc: 升序,desc: 降序 示例: "1:desc"
    ...
    */
    watch: {     //watch与计算属性的区别:watch可以执行异步操作,且watch可以通过配置deep:true来进行深度监视
      /* 
      ...
      */
      ...
      $route: {
        ...
        immediate: true, // 初始化时立即执行第一次,这样就可以不用在created中收集参数数据到options中并发送搜索的请求
      }
    },
    ...
    methods: {
      ...
      */
      setOrder (orderFlag) {
        // 得到当前的排序项和排序方式
        let [flag, type] = this.orderArr  //数组解构赋值
      ...
      */
      ...
      /* 
      添加一个属性条件,  根据属性值进行搜索
      点击某个属性值:如果已经存在条件数组中, 不添加.
                    向props数组添加一个条件字符串 子向父通信==>换种方式,使用vue自定义事件
    	              重新请求获取数据列表
      */
      addProp (prop) {
      ...
      */
      ...
      设置品牌条件 
      根据品牌进行搜索
      点击每个品牌:更新options中的trademark为指定的值,
                  重新请求获取数据列表. 
                  注意: 需要判断一下是否已经在条件了
      */
      setTrademark (trademark) {
        ...
        //data中的options为响应式对象,Vue无法探测普通的新增属性. 若一开始内部未定义trademark属性时,虽然this.options.trademark会添加新属性,但不会触发页面更新
        // 向响应式对象this.options中添加一个属性trademark，并确保这个属性同样是响应式的，且触发视图更新。
      ...
      */
      removeCategory () {
        ...
        /* 问题: 删除分类和关键字条件, 地址栏还有参数数据
	         原因: 只发送请求不会更改路径,删除时没有去除路径上的参数数据
	         解决: 重新跳转到search, 不再携带删除的条件所对应的参数 */
        // this.$router.push({
        this.$router.replace({
          name: 'search',
          // query: this.$route.query    //删除Category分类时,就不再携带query参数了
      ...
      */
      removeKeyword () {
        ...
        this.$router.replace({
          ...
          // params: this.$route.params      //删除Keyword时,就不再携带query参数了
        })

        // 3). 在Search中分发事件
        this.$bus.$emit('clearKeyword')//与header组件中绑定的事件监听对应
      },
